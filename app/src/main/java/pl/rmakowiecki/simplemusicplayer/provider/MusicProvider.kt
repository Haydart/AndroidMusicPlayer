...
class MusicProvider(private val context: Context) {
    val songList by lazy { retrieveDeviceSongList() }

    val albumList by lazy { retrieveDeviceAlbumList() }

    private fun retrieveDeviceSongList(): MutableList<Song> {
        ...
        musicResolver.query(EXTERNAL_CONTENT_URI, null, null, null, null).use {
            if (it != null && it.moveToFirst()) {
                ...
                do {
                    ...
                    list.add(Song(thisId, songTitle, songArtist, songAlbum, albumArtUri, songDuration.toInt()))
                } while (it.moveToNext())
            }
        }
    ...
    private fun retrieveDeviceAlbumList(): MutableList<Album> {
        ...
        musicResolver.query(MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI, projection, null, null, sortOrder).use {
            if (it != null && it.moveToFirst()) {
                ...
                do {
                    ...
                    list.add(Album(id, name, artist, ArrayList<Song>(), albumArtUri))
                } while (it.moveToNext())
            }
        }

        Collections.sort(list) { a, b -> a.artist.compareTo(b.artist) }
